...
const syncGithubButton = document.getElementById("syncGithub");
...
const checkGithubStatus = async () => {
  if (!syncGithubButton) return;

  try {
    const response = await fetch("/api/github/status");
    const status = await handleResponse(response);
    syncGithubButton.hidden = !status.configured;
    if (status.configured && status.organization) {
      syncGithubButton.title = `Sync teams from GitHub organization ${status.organization}`;
    } else {
      syncGithubButton.title = "Sync teams from GitHub";
    }
  } catch (error) {
    syncGithubButton.hidden = true;
  }
};
...
if (syncGithubButton) {
  syncGithubButton.addEventListener("click", async () => {
    const defaultLabel = "Sync from GitHub";
    syncGithubButton.disabled = true;
    syncGithubButton.textContent = "Syncing...";

    try {
      const response = await fetch("/api/github/sync", { method: "POST" });
      const summary = await handleResponse(response);
      const organization = summary.organization ? ` (${summary.organization})` : "";
      showToast(
        `Imported ${summary.teams} team${summary.teams === 1 ? "" : "s"} and ${summary.members} member${
          summary.members === 1 ? "" : "s"
        }${organization}.`
      );
      await fetchTeams();
      ...
      syncGithubButton.textContent = defaultLabel;
